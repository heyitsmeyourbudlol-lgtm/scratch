...
def _p2_verify_ok(ctx: dict[str, Any]) -> tuple[float, str]:
    ...
    if last.get("verify_ok") is True:
        ...
        if ft == "deferred":
            # Poison pair — fail-closed for factory meters; not a Phase-2 pass.
            return 0.0, "last_cycle.verify_ok=True with failure_type=deferred (poison)"
        return 1.0, "last_cycle.verify_ok=True"
    if last.get("verify_ok") is False:
        ft = str(last.get("failure_type") or "").strip()
        # OVERSEER_ASI_DEFERRED_SOFT_VERIFY_2026_09_07 — swarm/lock deferred is
        # soft Episodic (not red tests). Scoring 0.0 pinned harness at 42% while
        # factory was healthy-idle under continue_on_dirty.
        if ft == "deferred":
            return 1.0, "last_cycle soft deferred (swarm/lock) — not a red verify"
