...
def reclaim_cuda_allocator_cache(
    *,
    ...
    if soft and not force:
        ...
        if _LAST_EMBED_TS and now - _LAST_EMBED_TS < 8.0:
            return out
        # Need meaningful unused cache (not activation headroom).
        if slack < max(min_slack_mb, 8192.0):
            return out
        if not force and now - _LAST_CUDA_RECLAIM_TS < 90.0:
            return out
        # Keep allocated weights + activation pad; drop orphaned reserved only.
        # 2.8b-4bit sits ~1.8GB alloc with 18–32GB reserved — that slack must
        # return so ram_events can finish the footprint climb.
        working_keep = max(allocated + 4096.0, min(max(keep_mb, allocated + 6144.0), allocated + 12288.0))
        if reserved <= working_keep + 4096.0:
            return out
        # Only reclaim when MemAvailable is actually tight (otherwise leave cache).
        avail_gb = 99.0
        try:
...
def apply_gpu_health_cap(*, log_fn: Callable[[str], None] | None = None) -> float:
    ...
    cfg = _compute_cfg()
    try:
        # Teachers (790m / 2.8b-4bit) need real headroom. Default 0.28 starved
        # SM util after preload (needle: weights 100% then 0%/14–20W while
        # 70k–126k chunks pending). Prefer ~0.45 (~54GB) so productive encode
        # can heat GB10 while AnonPages still climb toward ~100GB.
        frac_max = float(cfg.get("cuda_memory_fraction_max") or 0.45)
    except (TypeError, ValueError):
        frac_max = 0.45
    try:
    ...
    # Emergency teacher_2 (2.8b-4bit) needs ~0.45–0.55; hard 0.32 starved SM util
    # after load (needle: weights 100% then 0%/14W while 123k 2.8b pending).
    frac_max = max(0.18, min(0.55, frac_max))
...
def effective_embed_batch_size() -> int:
    ...
    if override.get("embed_batch") is not None:
        try:
            # Never let a stale boost override (batch=256) starve emergency drain.
            return max(8, base, int(override["embed_batch"]))
        except (TypeError, ValueError):
            pass
    return base
    ...
    try:
        ...
        try:
            ...
            for mid, comp in ordered:
                batch_n = batch
                # 2.8b-4bit @ batch*0.85 left GB10 at ~0–10% SM / 14–20W while
                # 200k+ pending remained — heat the SMs, don't shrink.
                if "2.8b" in mid or comp in ("4bit", "q4", "int4"):
                    batch_n = max(batch, int(batch * 1.25), 384)
                elif "790m" in mid:
                    batch_n = max(batch, int(batch * 1.25))
...
def run_forever() -> None:
    ...
    try:
        ...
        if isinstance(dc, dict):
            ...
            dc["cuda_memory_fraction_max"] = max(
                min(float(dc.get("cuda_memory_fraction_max") or 0.45), 0.48), 0.40
            )
        ...
        try:
            ...
            try:
                ...
                if mode in ("emergency", "pressure", "boost"):
                    # Emergency/pressure: long productive bursts — not synthetic GEMM.
            ...
            # Soft reclaim on unified GB10 shrinks AnonPages (CUDA reserved IS
            # footprint) — never reclaim under target while teacher embed is hot.
            # Needle: 31608→1868MB reclaim → fp 89→79 + 0%/14W with 123k 2.8b left.
            try:
                import dgx_ram_budget as _rb
            ...
            # True OOM path only while under footprint target.
            if (not embed_hot) and fp < 97.0 and avail < 2.5:
                reclaim_cuda_allocator_cache(log_fn=_log, min_slack_mb=16384.0)
            elif fp >= 100.0 and avail < 6.0 and not embed_hot:
                reclaim_cuda_allocator_cache(log_fn=_log, min_slack_mb=8192.0)
            if bool(_compute_cfg().get("cuda_reclaim_enabled", False)) and (
                not embed_hot and (not _LAST_EMBED_TS or time.time() - _LAST_EMBED_TS >= 180.0)
            ):
                reclaim_cuda_allocator_cache(log_fn=_log, min_slack_mb=32768.0)
            try:
                status()
            except Exception:  # noqa: BLE001
                pass
