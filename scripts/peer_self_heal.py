...
def _peer_loop_process_pid() -> int | None:
    ...
    """
    return _forever_python_pid("peer_loop.py") or _forever_python_pid(
        "run_peer_loop_gitfile.py"
    )
...
def _job_stopped_loop_python_pids() -> list[tuple[str, int]]:
    ...
    for label, needles in (
        ...
        for needle in needles:
            pid = _forever_python_pid(needle)
            if pid is None:
                continue
            try:
                proc = subprocess.run(
                ...
                continue
            if not stat or stat[0] not in ("T", "t"):
                continue
...
def progress_fingerprint() -> dict[str, Any]:
    ...
    try:
        ...
        agents = sum(1 for ln in out.splitlines() if "cursor-agent" in ln)
    except (OSError, subprocess.CalledProcessError, subprocess.TimeoutExpired):
        agents = 0
...
def _scan_ram_ballast() -> list[str]:
    ...
    for line in out.splitlines():
        ...
        found.append(f"{pid_s}:{rss // 1024}MB:{name}")
    return found
