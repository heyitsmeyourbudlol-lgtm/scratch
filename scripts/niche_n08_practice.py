...
def predict(inp: Any) -> str:
    ...
    if "peer quiet" in both or ("poke no-op" in both and "no new lines" in both):
        return "peer_quiet"

    if "porcelain only notes/" in both or "dirty_tree_notes_only" in both:
        return "dirty_tree_notes_only"

    if "last_cycle poison" in both or (
        "verify_ok=true" in both and "failure_type=deferred" in both
    ):
        return "last_cycle_poison"
