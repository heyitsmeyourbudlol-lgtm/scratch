...
def _rank_input_key(
    ...
    *,
    ...
    try:
        ...
        team_key = ()
    findings_fp: tuple[int, int, int] = (0, 0, 0)
    try:
        import peer_dual_research as dr

        findings_fp = (
            _mtime_ns_safe(dr.FINDINGS_PATH),
            _mtime_ns_safe(dr.EFFICIENCY_DIGEST),
            _mtime_ns_safe(getattr(dr, "SYNC_DIGEST", Path())),
        )
    except Exception:  # noqa: BLE001
        findings_fp = (0, 0, 0)
...
def _maybe_compact_noop_heal(signals: ImproveSignals) -> None:
    ...
    if last.get("noop") or (last.get("verify_ok") and flat_fp and not last.get("local_only")):
        try:
