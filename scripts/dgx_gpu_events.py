...
def embed_productive() -> bool:
    ...
    try:
        for path in _gpu_compute_status_paths():
            ...
            cuda = st.get("cuda") or {}
            loaded = bool(mamba.get("loaded")) or float(cuda.get("memory_allocated_mb") or 0) > 128
            pending = int(st.get("chunks_pending") or 0)
            if loaded and fresh:
                return True
            # GB10 teacher preload: status may show loaded=true with 0 allocated
            # for 1–3min before first encode — still productive if queue pending.
            if fresh and pending > 0 and loaded:
                return True
            if fresh and pending >= 1000 and _gpu_compute_process_active():
                return True
...
def effective_util_pct(*, snap: dict[str, Any] | None = None) -> float:
    ...
    if embed_productive() and proxy < boost_util_pct() and _status_heartbeat_fresh(max_age_sec=90.0):
        power = float(snap.get("gpu_power_w") or 0)
        idle_w = util_proxy_power_idle_w()
        ...
        teacher_set = max(vram_mib, reserved_mb)
        if recent_hot and teacher_set >= 1500:
            proxy = max(proxy, target_util_pct())
            ...
            proxy = max(proxy, pressure_util_pct())
        elif embed_productive() and _gpu_compute_process_active():
            # Resident/pending Mamba between first encodes — leave emergency.
            # Needle: GB10 0%/5–14W after preload while 40k+ cascade pending.
            proxy = max(proxy, boost_util_pct())
...
def gpu_mode(*, snap: dict[str, Any] | None = None) -> str:
    ...
    # restart unloads 2.8b mid-queue).
    ...
    if embed_productive() and working and util >= boost_util_pct() - 1.0:
        return "hold"
    ...
    if embed_productive() and working:
        if scale < 0.35:
            return "hold"
        return "hold" if util >= boost_util_pct() - 1.0 else "boost"
...
def at_target(*, snap: dict[str, Any] | None = None) -> bool:
    ...
    # worker is mid-burst (power ~idle 12–16W, model loaded). Credit target when
    # productive + fresh embeds + effective util already at boost (proxy).
    if (
        embed_productive()
        and util >= target_util_pct() - slack
        and (
            float(snap.get("gpu_power_w") or 0) > util_proxy_power_idle_w() + 1.0
            or _recent_embed_activity(max_age_sec=45.0)
        )
    ):
        return True
    return False
...
def _infra_load_protect_active() -> bool:
    ...
    for path in (
        ...
        try:
            ...
            until = float((data or {}).get("protect_until") or 0)
            if until > time.time():
                return True
        except (OSError, ValueError, TypeError, json.JSONDecodeError):
            continue
...
def _restart_gpu_compute(log_fn: Callable[[str], None], *, force: bool = False) -> bool:
    ...
    if _gpu_compute_active():
        log_fn("gpu event: skip restart — dgx-gpu-compute already active")
        return False
    if _infra_load_protect_active() and _gpu_compute_process_active():
        log_fn("gpu event: skip restart — INFRA load protect + process live")
        return False
    state = _load_state()
    now = time.time()
