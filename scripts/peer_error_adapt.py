...
def _wake_loops(*, log_fn: Callable[[str], None] | None) -> None:
    try:
        ...
        improve.wake_peer(log_fn=log_fn or (lambda _m: None))
    except Exception:  # noqa: BLE001
        try:
            sig = auto.CONFIG_DIR / "peer-turn.signal"
            sig.parent.mkdir(parents=True, exist_ok=True)
            sig.write_text(str(time.time()), encoding="utf-8")
        except OSError:
            pass
    try:
        wake = auto.CONFIG_DIR / "improve-wake.signal"
        wake.parent.mkdir(parents=True, exist_ok=True)
        wake.write_text(str(time.time()), encoding="utf-8")
    except OSError:
        pass
