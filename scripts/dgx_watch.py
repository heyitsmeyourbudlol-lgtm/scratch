...
def _ssh(script: str, *, timeout: float = 25.0) -> tuple[int, str]:
    try:
        proc = subprocess.run(
            ["ssh", "-o", "BatchMode=yes", "-o", "ConnectTimeout=12", ssh_host(), script],
            capture_output=True,
            text=True,
            timeout=timeout,
            check=False,
...
def run_watch(*, heal: bool = True) -> dict[str, Any]:
    ...
    rc, ping = _ssh("echo ok", timeout=10.0)
    ...
    if not report["ssh_ok"]:
        ...
        if heal and heal_services_enabled():
