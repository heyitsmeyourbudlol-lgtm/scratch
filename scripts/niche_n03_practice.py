...
def predict(item: str) -> str | None:
    """Rule baseline for N03 — first claimable OVERSEER needle, else null."""
    text = (item or "").strip()
    if not text:
        return None
    m = _NEEDLE_LABELED_RE.search(text)
    if m:
        return m.group(1)
    m = _NEEDLE_TICK_RE.search(text)
    if m:
        return m.group(1)
    m = _NEEDLE_BARE_RE.search(text)
    if m:
        return m.group(1)
    return None
