...
def _sync_drift_lists(context_md: str, work_md: str) -> list[str]:
    ...
    if not ctx_items:
        ctx_items = auto._parse_phased_work_items(context_md)  # noqa: SLF001
...
def heal_queue_drift(*, root: Path, write: bool = False) -> tuple[list[str], list[str]]:
    ...
    # Remaining SoT for self-check (auto.sync_queue_drift) — not Active fallback.
    # OVERSEER_INSERT_CTX_REMAINING_ONLY_2026_09_07 — _sync_drift_lists may report
    # empty when ## Active already mirrors WQ while ## Remaining stays empty/missing;
    # always reconcile Remaining↔WQ opens so ISSUES clear.
