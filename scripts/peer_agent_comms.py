...
def glink_schema() -> dict[str, Any]:
    ...
    return {
        ...
        "validators": {
            ...
            "mcp_need": "mcp:<tool> | need=mcp + args.tool",
...
def _normalize_req_need(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if need.startswith(MCP_NEED_PREFIX):
        ...
        if not tool or " " in tool or len(tool) > 64:
...
def _format_msg_line(msg: dict[str, Any], *, line_cap: int | None = None) -> str:
    p = json.dumps(msg.get("p") or {}, separators=(",", ":"))
...
def format_glink_protocol_doc() -> str:
    return (
        "## GLink — agent comms (use instead of English between agents)\n\n"
        "Post **one JSON line** per update to the shared bus (append; do not rewrite history).\n"
        f"Bus file: `{_try_rel(BUS_PATH)}`\n"
        f"Your vault: `agents/{{role_id}}/state.json` under `{_try_rel(COMMS_DIR)}/`\n\n"
        "**Message shape** (compact keys):\n"
        "```json\n"
        '{"v":1,"t":"STAT","f":"factory_engineer","to":"*","ts":"ISO","p":{"st":"WIP","op":"adapt"}}\n'
        "```\n\n"
        "| t | meaning | p fields |\n"
        "|---|---------|----------|\n"
        f"| {MSG_STAT} | status broadcast | st=WIP\\|DONE\\|BLOCK\\|IDLE, op=short code |\n"
        f"| {MSG_DONE} | done | ref, ph[] |\n"
        f"| {MSG_REQ} | ask niche | need=code\\|mcp:<tool> |\n"
        f"| {MSG_ACK} | answer REQ | cid\\|ref |\n"
        f"| {MSG_BLOCK} | blocked | reason, wait? |\n"
        f"| {MSG_DIFF} | artifacts | ph[] (paths→ph; kp=1 keep) |\n"
        f"| {MSG_SUM} | summary | code\\|txt\\|sh{{m,f,q}}\\|ah |\n"
        f"| {MSG_ASN} | assign | id, task, eta, when, due |\n\n"
        "**Rules:** GLink bus = codes/ph only; vault = private todo; `to:*` broadcast. "
        "No English on bus."
    )
    ...
    *,
    ...
    if for_role is None:
        ...
        if not raw_lines:
            return (
                "## GLink bus (empty)\n\n"
                f"No messages yet — post to `{_BUS_PATH_STR}` after you change state.\n"
            )
        lines = ["## GLink bus (recent — all agents read this)", ""]
        for raw in raw_lines:
            clipped = raw
            if line_cap > 0 and len(clipped) > line_cap:
                clipped = clipped[: line_cap - 1] + "…"
            lines.append(f"- `{clipped}`")
        lines.append("")
        return "\n".join(lines)
    msgs = read_bus(limit=limit, for_role=for_role)
    if not msgs:
        return (
            "## GLink bus (empty)\n\n"
            f"No messages yet — post to `{_BUS_PATH_STR}` after you change state.\n"
...
def format_vault_block(role_id: str) -> str:
    if not comms_enabled():
        return ""
    ...
    lines = [
        f"### Vault `{role_id}`",
        f"- state: `{_try_rel(agent_state_path(role_id))}`",
        f"- todo: {len(todo)} · done: {len(done)}",
    ]
    if summary:
        lines.append(f"- summary: {summary[:SUMMARY_PROMPT_CAP]}")
    if todo[:3]:
        lines.append("- next: " + "; ".join(str(t)[:60] for t in todo[:3]))
    return "\n".join(lines)
...
def format_team_vaults(limit_roles: int = 8) -> str:
    ...
    blocks = [format_vault_block(r.id) for r in pool]
    blocks = [b for b in blocks if b]
    if not blocks:
        return ""
    return "## Agent vaults (per-niche state)\n\n" + "\n\n".join(blocks) + "\n"
