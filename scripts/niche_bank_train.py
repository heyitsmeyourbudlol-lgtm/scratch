...
def gen_all(catalog: list[dict[str, str]], *, skip_existing: bool = True) -> dict[str, Any]:
    ...
    for niche in catalog:
        nid = niche["id"]
        if nid == "N01" and skip_existing:
            skipped.append(nid)
            continue
        # Always keep curated N03/N08 JSONL; still allow neural stamp weights.
        if nid in ("N03", "N08") and skip_existing:
            train_p, held_p = jsonl_paths(nid, niche["name"])
            if train_p.is_file() and held_p.is_file():
                skipped.append(nid)
                # Still ensure practice dirs can train from existing JSONL via adapter below
