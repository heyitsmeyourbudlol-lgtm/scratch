...
def train_unlocked(*, root: Path | None = None) -> bool:
    ...
    Without train_unlock.json / UNLOCK needle + train_unlocked=true, returns False
    (TRAIN-LOCKED / T4 NO-GO / train_unlocked=false).
    """
    base = root or ROOT
    ...
    compact = recipe.replace(" ", "").replace(":", "").lower()
    if UNLOCK_NEEDLE in recipe and "train_unlocked=true" in compact:
        return True
    ready = _read(base / "notes" / "COMPRESSION_TRAIN_READY.md")
    if "train_unlocked=true" in ready.replace(" ", "").lower():
        # READY must not alone unlock; require unlock needle or artifact above.
        if UNLOCK_NEEDLE in ready or UNLOCK_NEEDLE in recipe:
            return True
    return False
