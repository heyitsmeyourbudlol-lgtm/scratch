...
- **output:** object:
  - `kit` (bool | null) — true if `[kit]` / kit-shaped; false if research/creative/defer/`[compression-train]`/`[research-speed]`/`[comms-improve]`/`[flaw-research]`/`[factory:…]`; null if ambiguous
  - `scope` (string) — primary path or module (`scripts/peer_loop.py`, `notes/BITNET_NICHE_BANK.md`, `scripts/compression_t4_*`, bare `COMPRESSION_*.md` → `notes/…`, bare `stress_bars.json` → `notes/compression_artifacts/stress_bars.json`) or `""` if none
    - Resolve: title / after em-dash **before** `AC:` path soup; prefer non-`tests/` over later AC `tests/*.py`; strip `./`; bare `peer_loop:` → `scripts/peer_loop.py`; include `.sh` primaries; bare compression artifact JSON → `notes/compression_artifacts/` (not `notes/<name>` alone)
...
| `peer_quiet` | peer log silent / poke no-op / hung event wait |
